...
ObjectId *task_return(task_spec *spec, int64_t ret_index) {
...
void print_task(task_spec *spec, UT_string *output) {
  ...
  for (int i = 0; i < task_num_returns(spec); ++i) {
    ObjectId *object_id = task_return(spec, i);
...
task_instance *make_task_instance(task_iid task_iid,
  ...
  task_instance *result = (task_instance*)malloc(size);
