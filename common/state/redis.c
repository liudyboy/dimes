...
db_handle *db_connect(const char *address,
                      ...
                      int client_port) {
  db_handle *db = (db_handle*)malloc(sizeof(db_handle));
  ...
  while (1) {
    reply = (redisReply*)redisCommand(context, "WATCH %s", client_type);
    freeReplyObject(reply);
    reply = (redisReply*)redisCommand(context, "HLEN %s", client_type);
    num_clients = reply->integer;
    freeReplyObject(reply);
    reply = (redisReply*)redisCommand(context, "MULTI");
    freeReplyObject(reply);
    reply = (redisReply*)redisCommand(context, "HSET %s %lld %s:%d", client_type,
...
void object_table_get_entry(redisAsyncContext *c, void *r, void *privdata) {
  db_handle *db = (db_handle*)(c->data);
  lookup_callback_data *cb_data = (lookup_callback_data*)privdata;
  redisReply *reply = (redisReply*)r;
  if (reply == NULL)
    return;
  int *result = (int*)malloc(reply->elements * sizeof(int));
  ...
  if (reply->type == REDIS_REPLY_ARRAY) {
    for (size_t j = 0; j < reply->elements; j++) {
      ...
      if (!entry) {
        redisReply *reply = (redisReply*)redisCommand(db->sync_context, "HGET %s %lld",
                                         db->client_type, result[j]);
        CHECK(reply->type == REDIS_REPLY_STRING);
        entry = (service_cache_entry*)malloc(sizeof(service_cache_entry));
  ...
  const char **manager_vector = (const char**)malloc(manager_count * sizeof(char *));
...
void object_table_lookup(db_handle *db,
                         ...
                         void *context) {
  lookup_callback_data *cb_data = (lookup_callback_data*)malloc(sizeof(lookup_callback_data));
...
void task_log_redis_callback(redisAsyncContext *c,
  ...
  CHECK(privdata);
  task_log_callback_data *callback_data = (task_log_callback_data*)privdata;
  task_instance *instance = (task_instance*)malloc(r->element[2]->len);
...
void task_log_register_callback(db_handle *db,
  ...
  task_log_callback_data *callback_data =
      (task_log_callback_data*)malloc(sizeof(task_log_callback_data));
