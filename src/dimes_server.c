/* DIMES SERVER: This is a simple object store server process
 *
...
UT_icd object_table_entry_icd = {sizeof(ObjectId), NULL, NULL, NULL};
...
plasma_store_state *init_plasma_store(event_loop *loop) {
  plasma_store_state *state = (plasma_store_state*)malloc(sizeof(plasma_store_state));
  state->loop = loop;
...
void create_object(client *client_context,
  ...
  HASH_FIND(handle, plasma_state->open_objects, &object_id, sizeof(ObjectId),
            entry);
  CHECKM(entry == NULL, "Cannot create object twice.");
  HASH_FIND(handle, plasma_state->sealed_objects, &object_id, sizeof(ObjectId),
            entry);
  CHECKM(entry == NULL, "Cannot create object twice.");
  ...
  memcpy(&entry->object_id, &object_id, sizeof(ObjectId));
  ...
  HASH_ADD(handle, plasma_state->open_objects, object_id, sizeof(ObjectId),
           entry);
...
int get_object(client *client_context,
  ...
  if (entry) {
    ...
    HASH_FIND(handle, plasma_state->objects_notify, &object_id,
              sizeof(ObjectId), notify_entry);
    if (!notify_entry) {
      /* notify_entry = (object_notify_entry*)malloc(sizeof(object_notify_entry)); */
      /* memset(notify_entry, 0, sizeof(object_notify_entry)); */
      void* tmp = malloc(sizeof(object_notify_entry));
      memset(tmp, 0, sizeof(object_notify_entry));
      notify_entry = (object_notify_entry*)tmp;

      utarray_new(notify_entry->waiting_clients, &client_icd);
      memcpy(&notify_entry->object_id, &object_id, sizeof(ObjectId));
...
void seal_object(client *client_context, ObjectId object_id) {
  ...
  HASH_ADD(handle, plasma_state->sealed_objects, object_id, sizeof(ObjectId),
           entry);
  ...
  object_notify_entry *notify_entry;
  HASH_FIND(handle, plasma_state->objects_notify, &object_id, sizeof(ObjectId),
            notify_entry);
  if (notify_entry) {
    plasma_reply reply;
    ...
    for (size_t i = 0; i < utarray_len(notify_entry->waiting_clients); ++i) {
...
void delete_object(client *client_context, ObjectId object_id) {
...
void send_notifications(event_loop *loop,
                        ...
                        void *context,
                        int events) {
  plasma_store_state *plasma_state = (plasma_store_state*)context;
   ...
   * possible. */
  for (size_t i = 0; i < utarray_len(queue->object_ids); ++i) {
    ObjectId *obj_id = (ObjectId *) utarray_eltptr(queue->object_ids, i);
    /* Attempt to send a notification about this object ID. */
    int nbytes = send(client_sock, obj_id, sizeof(ObjectId), 0);
    if (nbytes >= 0) {
      CHECK(nbytes == sizeof(ObjectId));
  ...
  utarray_erase(queue->object_ids, 0, (unsigned int)num_processed);
}
...
void start_server(const char *socket_name, plasma_store_state* state, event_loop* loop) {
...
void* start_rpc_server(void* state) {
  // tl::abt scope;
  auto host_ip = GetOwnIp();
  tl::engine myEngine((dimes::kRpcProtocol + "://" + host_ip + ":" + dimes::kRpcPort), THALLIUM_SERVER_MODE);
  auto plasma_state = static_cast<plasma_store_state*>(state);
  std::vector<tl::managed<tl::xstream>> ess;
  tl::managed<tl::pool> myPool = tl::pool::create(tl::pool::access::spmc);
  for (int i = 0; i < dimes::kRpcWorkerPoolSize; i++) {
  ...
  std::function<void(const tl::request & req, const std::string& object_name)>
      GetObject = [&plasma_state](const tl::request &req,
                                 const std::string& object_name) {
        ObjectId object_id(object_name);
        object_table_entry *entry;
        HASH_FIND(handle, plasma_state->sealed_objects, &object_id,
                  sizeof(ObjectId), entry);
        if (entry) {
          uint8_t *data =
              (uint8_t *)mmap(NULL, entry->map_size, PROT_READ | PROT_WRITE,
                              MAP_SHARED, entry->fd, 0) +
              entry->offset;
          dimes::ShareString request_data(data, entry->info.data_size);
          std::cout << "Start to respond data!" << std::endl;
          req.respond(request_data);
        } else {
          LOG_ERR("The data with id = %s is not exit in store.", object_name.c_str());
        }
      };

  myEngine.define("GetObject", GetObject, 0, *myPool);
...
int main(int argc, char *argv[]) {
  ...
  const char* socket_name = dimes::kSocketPath.c_str();
  LOG_DEBUG("starting server listening on %s", socket_name);
  event_loop *loop = event_loop_create();
  plasma_store_state *state = init_plasma_store(loop);
  pthread_t rpc_thread;
  pthread_create(&rpc_thread, NULL, start_rpc_server, (void*)state);
  start_server(socket_name, state, loop);
